...
*************************************
* draw binned scatter of household wages pre-retirement to household nonwage income post-retirement by race for balanced panel of retirees 
clear
use $home/data/hrs_cleaned.dta, replace 

*keep if inrange(age, 59,69)

*We're going to keep a sample of people for whom we observe for t-3 to t+2, where t is retirement 
keep if inrange(age - retage, -6,4)
egen valid = count(rwthh), by(hhidpn) /*count nonmissing obs */ 
keep if valid == 6
drop valid 


*now for these people we will collapse their longitudinal data into one row, with their mean pre-retirement household wsi , mean post-retirement household nonwage income, age at retirement, and mean weight over this period. 
gen hiearn_adj = hiearn/PCEPI if age < retage
gen hinw_adj = hinw_idda/PCEPI if age >= retage

collapse (mean) hiearn_adj hinw_adj retage rwthh, by(hhidpn xred) 



keep if xred != 3 & hiearn_adj >= 5000 /*we're going to censor some observations with very low earnings prior to retirement. You can remove this and it doesn't change much  */ 
xtile wage_quintile = hiearn_adj [aw = rwthh], n(5) /*bin by across-race terciles of pre-retirement wages*/
collapse (mean) hinw_adj (mean) hiearn_adj (count) hhidpn [aw= rwthh] if xred != ., by(xred wage_quintile)
...
//note("Source: HRS wave 5+, individuals must be observed for 6 periods centered around retirement. " " Mean pre-retirement earnings must be above $5,000. Data is binned into 5 equal-sized groups.")  

graph export "$home/graphs/figure_7.png", name(scatter_y_prepostretirement) replace 

rename hinw_adj nonwage_income
rename hiearn_adj wage_income
drop wage_quintile hhidpn
export delimited "$home/out/figure_7", replace

***********************************************************************
* Draw line chart of Pr(WSI >0 | htot_idda quartile) over time
use $home/data/hrs_cleaned.dta, replace 
 ...
 * foreach year, calculate your quartile of household income 
...
keep if py0_htot != .
collapse (mean) pos_wsi (mean) age (count) hhidpn [aw = rwthh], by(year py0_htot)

twoway 	(connected pos_wsi year if py0_htot == 1) ///
		(connected pos_wsi year if py0_htot == 4), ///
...
**********************************************
*draw bar chart of mean nwi share of hhincome by total hh income quartile using pooled hrs sample 
